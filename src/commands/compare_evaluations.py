...
def setup_metrics_table(metrics, kind):
    ...
    if kind == "dl":
        ...
        metrics["seq_len"] = metrics.model.apply(
            lambda x: int(x.split("_")[2].split("seq-len")[0])
        )
        metrics["steps"] = metrics.model.apply(
            lambda x: int(x.split("_")[3].split("steps")[0])
        )
        metrics.sort_values(["network", "seq_len", "steps"])
    elif kind == "ml":
        metrics.model = metrics.model.apply(
            lambda x: x.replace("random_", "random").replace("logistic_", "logistic")
        )
    ...
    if model is not None:
        ...
        if len(data) > 0:
            for val in values:
                ...
                #                 sns.heatmap(plot)
                plot = data[["seq_len", "steps", val]].pivot(
                    index="seq_len", columns="steps", values=val
                )
                plt.title(f"{model.upper()} - {val.upper()}")
                for col in plot:
        ...
        if len(data) > 0:
            for val in values:
                ...
                #                 plt.title(f"{model.upper()} - {val.upper()}")
                #                 sns.heatmap(plot)
                data["myindex"] = data[["network", "seq_len"]].apply(
                    lambda x: f"{x.network}_{x.seq_len}_", axis=1
                )
                plot = data[["myindex", "steps", val]].pivot(
                    index="myindex", columns="steps", values=val
                )
                for i in range(10):
                    plt.title(f"ALL - {val.upper()} - {i+1} Sequence Length")
                    for idx, row in plot[
                        plot.index.str.contains(f"_{i+1}_")
                    ].iterrows():
    ...
    if model is not None:
        ...
        if len(data) > 0:
            for val in values:
                ...
                #                 plt.title(f"{model.upper()} - {val.upper()}")
                #                 sns.heatmap(plot)
                data["myindex"] = data[["ml_model", "steps"]].apply(
                    lambda x: f"{x.ml_model}_{x.seq_len}_", axis=1
                )
                plot = data[["myindex", val]].pivot(
                    index="myindex", columns="steps", values=val
                )
                for i in range(10):
                    plt.title(
                        f"ALL - {val.upper().replace('_', '-')} - {i+1} Sequence Length"
                    )
                    for idx, row in plot[
                        plot.index.str.contains(f"_{i+1}_")
                    ].iterrows():
...
def compare_evaluations(glob_, output_directory, kind):
    output_directory = create_directory(output_directory)

    LOGGER.info("Reading metric files ...")
    metrics = read_multiple_csvs(get_files(glob_))
