...
def prepare_netflow_sequantial_data(
    ...
    preprocess_features = Pipeline(
        steps=[
            (
                "ExtractFeatures",
                FunctionTransformer(
                    lambda x: x[:, :-1] if remove == 0 else x[:-remove, :-1],
                    validate=True,
                ),
            ),
            (
    ...
    preprocess_labels = Pipeline(
        steps=[
            (
                "ExtractLabels",
                FunctionTransformer(
                    lambda x: (x[:, -1] if remove == 0 else x[:-remove, -1]).reshape(
                        -1, 1
                    ),
                    validate=True,
                ),
            ...
            (
                "Reshaping",
                FunctionTransformer(
                    lambda x: x.reshape((-1, seq_len, 4 if transition == 2 else 2)),
                    validate=True,
                ),
            ),
            ...
            (
                "Align",
                FunctionTransformer(
                    lambda x: x[forward_predict:, :, :].reshape(
                        -1, seq_len, 4 if transition == 2 else 2
                    ),
                    validate=False,
                ),
            ),
